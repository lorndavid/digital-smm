...
import { ArrowDownWideNarrow, FolderTree, Layers, Pencil, Plus, Search, Trash2 } from '@lucide/vue'
    ...
    <div class="glass grid gap-3 rounded-2xl p-4 shadow-card sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div class="flex items-end">
        <BaseButton
          variant="ghost"
          class="w-full"
          @click="search = ''; showEmpty = false; sort = 'count'; page = 1; void load()"
        >
            ...
            <tr v-for="category in items" :key="category._id" class="transition-colors hover:bg-(--a-hover)">
              ...
              <td class="px-5 py-3.5">
                <div class="flex justify-end gap-2">
                  <button
                    class="flex h-8 w-8 items-center justify-center rounded-lg text-(--a-muted) hover:bg-(--a-hover) hover:text-(--a-text)"
                    aria-label="Edit"
