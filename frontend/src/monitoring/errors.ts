...
export function installGlobalErrorHandlers(): void {
  ...
  const hasErrorListeners = window.__DIGITALSMM_ERROR_HOOKS_INSTALLED__
  if (hasErrorListeners) return
  window.__DIGITALSMM_ERROR_HOOKS_INSTALLED__ = true
  ...
  // Report the environment tag with every capture.
  Sentry.setTag('app_environment', analyticsConfig.environment)
}

declare global {
  interface Window {
    __DIGITALSMM_ERROR_HOOKS_INSTALLED__?: boolean
  }
}
