  ...
  <section id="pricing" class="mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
    ...
    <div class="mt-14 grid gap-6 lg:grid-cols-3">
      <div
        ...
        :class="plan.highlight ? 'bg-gradient-to-b from-brand-500 to-secondary-500 shadow-glow lg:-translate-y-3' : 'bg-ink/10 hover:-translate-y-1'"
      >
        <div class="glass-strong h-full rounded-3xl p-7">
          ...
          <p class="mt-1 text-sm text-ink/50">{{ plan.description }}</p>
          <p class="font-display mt-5 text-4xl font-bold text-ink">
            {{ plan.price }}<span v-if="plan.price !== 'Custom'" class="text-base font-medium text-ink/40"> / start</span>
          </p>
          <ul class="mt-6 space-y-3">
            <li v-for="feature in plan.features" :key="feature" class="flex items-start gap-2.5 text-sm text-ink/70">
