...
<template>
  <header
    class="glass-strong sticky top-0 z-30 flex h-16 items-center gap-2 border-b border-line px-4 sm:gap-3 sm:px-6"
  >
    <!-- Mobile: menu + brand -->
    <button
      class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-ink/70 transition-colors hover:bg-ink/5 lg:hidden"
    ...
    <div class="hidden min-w-0 lg:block">
      <p class="truncate font-display text-base font-semibold text-ink">{{ pageTitle }}</p>
    </div>

    <div class="ml-auto flex items-center gap-2 sm:gap-3">
      <!-- Wallet balance chip -->
      <button
        class="flex h-9 items-center gap-1.5 rounded-xl border border-line bg-soft px-2.5 text-sm font-semibold text-ink/80 transition-colors hover:border-emerald-400/40 hover:text-ink sm:px-3"
